...
export class CsClockComponent implements AfterContentInit,AfterViewChecked {
  ...
  private drawClockBack() {
    ...
    let lineGradient = this.canvasContext.createLinearGradient (this.radius, this.radius, this.radius +50, this.radius +50);
    ...
    let numberWidth = this.canvasContext.measureText("Board").width;
  ...
  private drawNumbers() {
    ...
    let angle: number = 0, numberWidth: number = 0;
    clockNumbers.forEach(value => {
      angle = Math.PI / 6 * (value - 3);
      numberWidth = this.canvasContext.measureText(value.toString()).width;
      let number_X = this.radius + Math.cos(angle) * (this.radius * 0.8) - numberWidth / 2;
      let number_Y = this.radius + Math.sin(angle) * (this.radius * 0.8) ;
